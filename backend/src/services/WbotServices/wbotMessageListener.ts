  ...
  try {
    ...
    const types = {
      ...
      protocolMessage:
        ...
        msg.message?.ephemeralMessage?.message?.protocolMessage?.type,
...
const handleChartbot = async (
  ...
  if (!isNil(queue) && !isNil(ticket.queueOptionId) && messageBody == "#") {
    ...
    // não listar a primeira pergunta
  } else if (
    !isNil(queue) &&
  ...
  try {
    ...
    if (isGroup) {
      try {
        ...
        if (
          bdTemp[msg.key.remoteJid] &&
          bdTemp[msg.key.remoteJid]?.grupo?.createdAt &&
          bdTemp[msg.key.remoteJid].grupo
        ) {
        ...
        if (bodyMessage && bodyMessage.toUpperCase() === "##KEYGROUP") {
          const sentMessage = await wbot.sendMessage(`${msg.key.remoteJid}`, {
            text: `\u200eGrupo id: ${grupoMeta.id} \nNome: ${
              ...
              grupoMeta.participants.length
            }\n \nParticipantes:\n${grupoMeta.participants
              .map(
                (participant: { id: any; admin: any }) =>
                  `${participant.id} ${
    ...
    if (
      ...
      formatBody(whatsapp.farewellMessage, contact).toString() ===
        formatBody(bodyMessage, contact).toString()
    ) {
      return;
    }
    ...
    try {
      if (!msg.key.fromMe && scheduleType && !ticket.queueId && !ticket.userId) {
        /**
  ...
  try {
    ...
    wbot.ev.on("message-receipt.update", (events: any) => {
      events.forEach(async (msg: any) => {
        const ack = msg?.receipt?.receiptTimestamp
          ? 3
          : msg?.receipt?.readTimestamp
          ? 4
          : 0;
        if (!ack) return;
